(function (root, factory) {
    ...
    Ssi_upload.prototype.init = function (element) {
        ...
        $clearBtn.click(function () { //选择已完成文件和待处理文件
            thisS.clear();
        });

        var $tooltip;

        $uploadBox.on('mouseenter', '.ssi-statusLabel', function (e) { //工具提示
        ...
        $uploadBox.on('click', '.ssi-removeBtn', function (e) { //删除单一文件
            var $currentTarget = $(e.currentTarget);
            var index = $currentTarget.data('delete'); //获取文件下标
            thisS.pending--; //将待处理的数量减少1
            thisS.currentListLength--; //将当前列表长度减少1
            if (thisS.pending === 0) {
                $uploadBtn.prop('disabled', true); //如果没有更多文件，则禁用上载按钮
            }
            if (thisS.options.preview) { //如果预览为真
                $currentTarget.parents('table.ssi-imgToUploadTable').remove(); //remove table
            } else {
                var target = $currentTarget.parents('tr.ssi-toUploadTr'); //找到文件的tr
                $namePreview.html((thisS.currentListLength) + ' files'); //将主名称设置为其余文件
                target.prev().remove();// 删除空tr（使用id作为行之间的边距）
                target.remove();// 删除文件
                if (thisS.currentListLength === 1) { //如果列表中仅剩一个文件
                    setLastElementName(thisS); //设置主预览以显示名称
                }
            }
            thisS.toUpload[index] = null; //将文件的obj设置为null（因为需要保持相同的索引，所以不对其进行拼接）
            thisS.imgNames[index] = null; //将文件名设置为null

            if (thisS.currentListLength === 0) { //如果列表中没有更多文件
                if (!thisS.options.dropZone) { // 如果禁用了拖放
        ...
        $uploadBox.on('click', '.ssi-abortUpload', function (e) {//中止一个元素
            var $eventTarget = $(e.currentTarget);
            var index = $eventTarget.data('delete');//获取元素ID
            thisS.abort(index); // 中止请求
        });
        
//----------------------------UPLOADFILES------------------------------------
        $uploadBtn.click(function () {// 上传文件
            thisS.uploadFiles();
        });
        $abortBtn.click(function () { // 中止所有请求
            thisS.abortAll();
        });
    ...
    Ssi_upload.prototype.abortAll = function () {
        for (var i = 0; i < this.uploadList.length; i++) { //列表中的所有元素
            if (typeof this.uploadList[i] === 'object') {// 检查是否删除
                this.abort(i)
            }
        }
    };
    Ssi_upload.prototype.toUploadFiles = function (files) {
        if (typeof this.options.maxNumberOfFiles === 'number') {
            if ((this.inProgress + this.pending) >= this.options.maxNumberOfFiles) {// 如果正在进行中的文件+待处理的文件大于,定义为预下载的最大文件数
                return;//什么都不做
            }
        }
        var thisS = this,
         ...
         imgs = [];
        if ((this.inProgress === 0 && this.pending === 0)) { //如果没有文件待处理或正在进行中
            this.clear(); //清空列表
        }
        ...
        for (var i = 0; i < filesLength; i++) {
            ...
            } else if ((file.size * Math.pow(10, -60)).toFixed(2) > this.options.maxFileSize) {//if file size is more than we ask
    ...
    $.fn.ssi_uploader = function (opts) {
        var defaults = {
            ...
            allowed: ['jpg', 'jpeg', 'png','txt','doc','docx','xls','xlsx'],
            errorHandler: {
                method: function (msg) {
                    alert(msg);
                },
    ...
    var locale = {
    	cn: {
            ...
            clear: '清空所有',
            drag: '拖拽上传',
            sizeError: '$1 超出大小限制 $2',// $1=file name ,$2=max ie( example.jpg has has exceed the size limit of 2mb)
            extError: '$1 文件类型错误',//$1=file extension ie(exe files are not supported)
